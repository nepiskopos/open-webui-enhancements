...
def summarize_file(
    ...
    """
    Send a request to the Open WebUI API to summarize the content of uploaded files.
    ...
    prompt = "Please summarize the content of the uploaded file."
...
def main():
    ...
    parser = ArgumentParser(
        prog='SummarizationTest',
        description='Test the summarization of a DOCX file using the Summarization Pipeline (+ Filter) via Open WebUI API.',
    )
    ...
    parser.add_argument('-m', '--model', metavar='MODEL', default="summarization-pipeline",
                        help='Model to use for summarization (default: summarization-pipeline)', type=str)
    parser.add_argument('-f', '--file', metavar='FILE', help='Path to the DOCX file to summarize', type=str, required=True)
    parser.add_argument('-o', '--output', metavar='OUTPUT', help='Optional file to save the summarization result', type=str)
    ...
    try:
        ...
        # Prepare file info for summarization
        files = [{"type": "file", "file": upload_info}]

        # Request summarization
        print("Requesting summarization...")
        result = summarize_file(args.url, args.token, args.model, files)
        ...
        if result:
            ...
            summary_text = result[0]["summary"] if isinstance(result, list) else result["summary"]
            print(summary_text)
            print("=" * 80)
            ...
            print("Error: Failed to get summarization result")
            return 1
